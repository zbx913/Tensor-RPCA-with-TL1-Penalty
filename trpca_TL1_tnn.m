function [L,S,err,iter] = trpca_TL1_tnn(X,lambda,opts)

% Solve the Tensor Robust Principal Component Analysis based on Tensor TL1 Nuclear Norm problem by ADMM
%
% min_{L,S} ||L||_*TL1+lambda*||S||_1, s.t. X=L+S
%
% ---------------------------------------------
% Input:
%       X       -    d1*d2*d3 tensor
...
% 
%
% Based on  the code of  Canyi Lu as follows:
% 
% References: 
...
for iter = 1 : max_iter
    ...
    Sk = S;
    % update L
    [L,tnnL] = prox_TL1_tnn(-S+X-Y/mu,1/mu);
    % update S
